...
- **모니터 켜진 후 안정화 대기 추가**
  - 디스플레이 활성화 감지 즉시 캡쳐 → 5초 대기 후 캡쳐
  - 외장 모니터(HDMI 등) 화면 출력 완전 안정화 확보
...
- **모니터 잠자기 해제 미작동 버그 수정**
  - `IOPMAssertionDeclareUserActivity` → `caffeinate -u -t 10` 으로 교체
  - `IOPMAssertion`은 잠자기 **방지** API로 이미 꺼진 디스플레이를 켜지 못함
  - `caffeinate -u`는 꺼진 디스플레이를 실제로 켜도록 설계된 표준 유틸리티
  - `IOKit.pwr_mgt` import 제거

---

## [v0.2.0] - 2026-03-27 - 디스플레이 전력 관리 기능 추가
