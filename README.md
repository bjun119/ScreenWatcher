...
- **Variational 재인증 알림**: 7일 주기 재인증 만료 전 단계별 텔레그램 알림 (v0.3.0+)
- **로그인 시 자동 시작**: LaunchAtLogin 지원
- **메뉴바 통합**: 빠른 접근 및 상태 확인

## 버전

- **최신**: v0.3.1 (2026-03-28)
  - Variational 인증 완료 버튼 오작동 방지 확인 다이얼로그 추가
  - 설정 화면 경과 시간 실시간(1초) 갱신
  - 메뉴바 클릭 시 경과 시간 즉시 갱신

- v0.3.0 (2026-03-28)
  - Variational 재인증 만료 단계별 텔레그램 알림 기능 추가
  - 알림음 켜기/끄기 설정 추가
  - 메뉴바에 인증 경과 시간(D-day) 표시
...
빌드 완료 후 `ScreenWatcher-v0.3.1-arm64.zip` 파일이 생성됩니다.
...
| `variationalNotificationSound` | true | Variational 알림음 켜기 |
| `variationalLastAuthDate` | - | 마지막 Variational 인증 일시 |
...
| Schedule | 스케줄 실행/스킵 |
| Telegram | API 호출 결과 |
| WalletReminder | Variational 재인증 알림 전송 결과 |
